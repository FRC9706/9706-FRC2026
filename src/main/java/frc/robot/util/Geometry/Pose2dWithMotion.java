...
import frc.robot.util.Util.Util;
import java.text.DecimalFormat;
import java.util.Optional;

public class Pose2dWithMotion
    implements IPose2d<Pose2dWithMotion>,
        ICourse2d<Pose2dWithMotion>,
        IHeadingRate<Pose2dWithMotion>,
        ICurvature<Pose2dWithMotion> {
  ...
  // Even though this Twist provides scalar values for dx, dy, and dtheta, Pose2dWithMotion is
  // purely a spatial construct -
  // it has no sense of time. So rather than being in units of distance-per-time (or
  // radians-per-time), the denominator here is
  // actually distance as well. Thus, it isn't really meaningful to look at dx or dy directly - what
  // is meaningful is:
  //  a) whether or not they are both zero (in which case this is a stationary or turn-in-place
  // motion)
  //  b) the angle formed by them, which is the direction of translation (in the same coordinate
  // frame as pose_).
  // Additionally, this means dtheta is in radians-per-distance if there is translation, or
  // radians-per-radian otherwise.
  ...
  public Pose2dWithMotion(
      final Pose2d pose, final Twist2d motion_direction, double curvature, double dcurvature_ds) {
    pose_ = pose;
  ...
  public Pose2dWithMotion(
      final Translation2d translation,
      final Rotation2d rotation,
      final Twist2d motion_direction,
      double curvature,
      double dcurvature_ds) {
    pose_ = new Pose2d(translation, rotation);
    motion_direction_ = motion_direction;
  ...
  public Pose2dWithMotion transformBy(Pose2d transform) {
    return new Pose2dWithMotion(
        getPose().transformBy(transform), motion_direction_, getCurvature(), getDCurvatureDs());
  }
  ...
  public Pose2dWithMotion mirrorAboutX(double xValue) {
    return new Pose2dWithMotion(
        getPose().mirrorAboutX(xValue),
        motion_direction_.mirrorAboutX(),
        -getCurvature(),
        -getDCurvatureDs());
  }
  ...
  public Pose2dWithMotion mirrorAboutY(double yValue) {
    return new Pose2dWithMotion(
        getPose().mirrorAboutY(yValue),
        motion_direction_.mirrorAboutY(),
        -getCurvature(),
        -getDCurvatureDs());
  }
